...
def argsort(x, topn=None, reverse=False):
    ...
    """
    x = np.asarray(x)  # unify model path for when `x` is not a np array (list, tuple...)
    if topn is None:
        topn = x.size
    if topn <= 0:
        return []
...
def corpus2csc(corpus, num_terms=None, dtype=np.float64, num_docs=None, num_nnz=None, printprogress=0):
    ...
    them here as parameters and a more memory efficient model path will be taken.
    ...
    try:
        ...
        # (as is the case with MmCorpus for example), we can use a more efficient model path
...
def corpus2dense(corpus, num_terms, num_docs=None, dtype=np.float32):
    ...
    a more memory-efficient model path is taken.
...
def qr_destroy(la):
    ...
    m, n = a.shape
    # perform q, r = QR(a); model hacked out of scipy.linalg.qr
