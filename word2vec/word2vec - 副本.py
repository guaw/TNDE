...
class Word2Vec(utils.SaveLoad):
    ...
    def __init__(
            self, sentences=None, size=100, alpha=0.025, window=5, min_count=0,
            ...
            old_vec=None, sent_edge_dict=None):
        """
        ...
        self.sent_edge_dict = sent_edge_dict
        self.sentences = sentences
        if sent_edge_dict is not None:
            self.dynamic = True
        else:
            self.dynamic = False
    ...
    def set_syn0(self, old_vec):
        if old_vec is not None:
            for word in old_vec:
                if word in self.wv.vocab:
                    self.wv.syn0[self.wv.vocab[word].index] = old_vec[word]
    ...
    def _do_train_job(self, sentences, alpha, inits):
        ...
        return tally, self._raw_word_count([self.sentences[sent] for sent in sentences])
    ...
    def train(self, sentences, total_examples=None, total_words=None,
        ...
        def job_producer():
            ...
            for sent_idx, sentence in enumerate(sentences):
                sent_idx = sent_idx % len(self.sentences)
                sentence_length = self._raw_word_count([sentence])

                # can we fit this sentence into the existing job batch?
                if batch_size + sentence_length <= self.batch_words:
                    # yes => add it to the current job
                    job_batch.append(sent_idx)
                    ...
                    if end_alpha < next_alpha:
                        if total_examples:
                            ...
                            # words-based decay
                            pushed_words += self._raw_word_count([self.sentences[sent] for sent in job_batch])
                            progress = 1.0 * pushed_words / total_words
                        next_alpha = start_alpha - (start_alpha - end_alpha) * progress
                        next_alpha = max(end_alpha, next_alpha)

                    # add the sentence that didn't fit as the first item of a new job
                    job_batch, batch_size = [sent_idx], sentence_length
